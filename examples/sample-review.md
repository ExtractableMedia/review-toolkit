...
**Issue:** User input is interpolated directly into a SQL query without
sanitization.
...
**Issue:** Credit card numbers are logged in plaintext during transaction
processing.
...
**Issue:** The test suite thoroughly covers boundary conditions for user
validation, including Unicode handling and max-length edge cases.

### F7 🟡 Medium Priority - Missing test for error path

**File:** `spec/services/payment_processor_spec.rb`
**Issue:** No test covers the case where the payment gateway returns a
timeout error.
