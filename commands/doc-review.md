...
Use these severity conventions for quick visual scanning:
...
preserving the original content for reference. Observation findings do not require status tracking.
...
1. **Checklist** - Convert actionable findings (🔴🟠🟡🟢) into a checklist. Do not include Observation
   findings in the checklist.
...
When posting review findings as a PR comment (e.g., when explicitly asked), build a temporary file
with a collapsible `<details><summary>` wrapper and post it with `--body-file` to avoid heredoc
quoting issues:
